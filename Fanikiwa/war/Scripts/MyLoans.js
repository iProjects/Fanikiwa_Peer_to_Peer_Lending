...
fanikiwa.loanendpoint.myloans = fanikiwa.loanendpoint.myloans || {};

fanikiwa.loanendpoint.myloans.LoadLoans = function() {
	...
	gapi.client.loanendpoint.selectDtoMyLoans({
		'email' : email
	}).execute(
			function(resp) {
 ...
 */
fanikiwa.loanendpoint.myloans.init = function(apiRoot) {
	// Loads the APIs asynchronously, and triggers callback
	// when they have completed.
	var apisToLoad;
	var callback = function() {
		if (--apisToLoad == 0) {
			fanikiwa.loanendpoint.myloans.LoadLoans();
		}
	}
		...
		for (var i = 0; i < resp.result.items.length; i++) {
			if (resp.result.items[i] != null) {
				...
				loanTable += '<td>'
						...
						+ '</td>';
				loanTable += '<td>' + resp.result.items[i].lender + '</td>';
				...
				loanTable += '<td><a href="#" onclick="Repay('
						+ resp.result.items[i].id + ')">Repay</a> </td>';
...
function Repay(id) {
	sessionStorage.loanrepaymentid = id;
	window.location.href = "/Views/Loans/LoanRepayment.html";
}
