...
public class JobAccrueInterest implements IJobItem {

	boolean EnableLog = true;

	private static final Logger log = Logger.getLogger(JobAccrueInterest.class.getName());
	...
	private int GetAccountTerm(Account acc, Date date) //return term in days
	{
		...
		/*
		String term = acc.getInterestComputationTerm();
		if(StringExtension.isNullOrEmpty(term) ) return 1;
		...
		case "M": return DateExtension.DaysOfTheMonth(date); 
		case "M30": return 30;
		case "Y": return DateExtension.DaysOfTheYear(date);
		default: return 1;
		}
		*/
	}
