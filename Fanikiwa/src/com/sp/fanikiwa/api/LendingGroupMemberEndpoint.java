...
public class LendingGroupMemberEndpoint {
	...
	public CollectionResponse<Lendinggroupmember> listLendinggroupmember(
		...
		Query<Lendinggroupmember> query = ofy().load().type(
				Lendinggroupmember.class);
	...
	public CollectionResponse<Lendinggroupmember> selectgroupMembers(
		...
		Query<Lendinggroupmember> query = ofy().load()
				.type(Lendinggroupmember.class).filter("groupName", Groupname);
		return GetLendinggroupmembersFromQuery(query, cursorString, count);
	}

	private CollectionResponse<Lendinggroupmember> GetLendinggroupmembersFromQuery(
			Query<Lendinggroupmember> query, String cursorString, Integer count) {
		...
		return CollectionResponse.<Lendinggroupmember> builder()
	...
	public Lendinggroupmember getLendinggroupmemberByID(
			@Named("groupId") Long id) {
	...
	public Lendinggroupmember updateLendinggroupmember(
	...
	public void removeLendinggroupmember(@Named("id") Long id)
			throws NotFoundException {
	...
	public Lendinggroupmember insertLendinggroupmember(
			Lendinggroupmember Lendinggroupmember) throws NotFoundException,
			ConflictException {
